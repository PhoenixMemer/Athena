...
# 🌐 CONFIGURATION
# ==========================================
BUSINESS_CHANNEL_ID = 1400515374977650799  # Set your #wall-street or #business channel
DB_PATH = "business.db"
ECO_DB = "economy.db"

# Sector-Specific Product Catalogs (Fixes "invalid product" error)
SECTOR_CATALOGS = {
...
class BoardMeetingModal(discord.ui.Modal):
    def __init__(self, crisis_type: str, options: list):
        super().__init__(title=f'CRISIS: {crisis_type}')
        self.options = options
        self.choice = discord.ui.TextInput(label='Your Decision', placeholder='Type 1, 2, or 3', max_length=2)

    async def on_submit(self, i: discord.Interaction):
        choice = self.choice.value.strip()
        if choice not in ['1', '2', '3']:
            return await i.response.send_message("❌ Invalid choice. Use 1, 2, or 3.", ephemeral=True)
        
        idx = int(choice) - 1
        outcome = self.options[idx]
        
        with get_db_cursor() as c:
            if 'capital' in outcome:
                atomic_business_update(c, i.user.id, outcome['capital'])
            if 'rep' in outcome:
                c.execute("UPDATE businesses SET reputation = MIN(100, MAX(0, reputation + ?)) WHERE user_id = ?", (outcome['rep'], i.user.id))
            if 'tech' in outcome:
                c.execute("UPDATE businesses SET tech_level = tech_level + ? WHERE user_id = ?", (outcome['tech'], i.user.id))
            log_business_event(c, i.user.id, "BOARD_MEETING", f"Resolved {outcome.get('desc', 'crisis')}")
        
        await i.response.send_message(f"✅ Board decision applied: {outcome.get('desc', 'Crisis managed.')}", ephemeral=True)
...
class EarningsCallView(discord.ui.View):
    ...
    @discord.ui.button(label="📈 Beat Expectations", style=discord.ButtonStyle.primary)
    async def beat(self, i: discord.Interaction, button): self._resolve(i, "beat")
    @discord.ui.button(label="📊 Meet Expectations", style=discord.ButtonStyle.secondary)
    async def meet(self, i: discord.Interaction, button): self._resolve(i, "meet")
    @discord.ui.button(label="📉 Miss Expectations", style=discord.ButtonStyle.danger)
    async def miss(self, i: discord.Interaction, button): self._resolve(i, "miss")

    def _resolve(self, i: discord.Interaction, outcome: str):
        for child in self.children: child.disabled = True
        mod = {"beat": 1.15, "meet": 1.0, "miss": 0.85}[outcome]
        with get_db_cursor() as c:
            c.execute("SELECT capital FROM businesses WHERE user_id = ?", (self.user_id,))
            cap = c.fetchone()[0] or 0
            tax = int(cap * 0.10)
            atomic_business_update(c, self.user_id, -tax)
            c.execute("INSERT OR REPLACE INTO quarterly_reports (user_id, quarter, revenue_modifier) VALUES (?, ?, ?)", (self.user_id, self.quarter, mod))
            log_business_event(c, self.user_id, "EARNINGS_CALL", f"Q{self.quarter} {outcome} expectations")
        msg = {"beat": "🎉 Investors thrilled! Stock price boosted.", "meet": "Stable quarter. Market holds steady.", "miss": "📉 Shareholders disappointed. Reputation takes a hit."}[outcome]
        i.response.edit_message(content=f"📞 Earnings Call Complete: {msg}\nTaxes paid: A$ {tax:,}", view=self)
        self.stop()

class ProductDropdown(discord.ui.Select):
    def __init__(self, sector: str):
        self.sector = sector
        self.catalog = SECTOR_CATALOGS.get(sector, [])
        opts = [discord.SelectOption(label=p, value=p) for p in self.catalog]
        super().__init__(placeholder="Select a product to launch...", options=opts)

    async def callback(self, i: discord.Interaction):
        await i.response.send_modal(ProductModal(self.sector))
...
class OpsView(discord.ui.View):
    ...
    async def prod(self, i: discord.Interaction, btn):
        ...
        if not row or not row[0]:
            return await i.response.send_message("❌ Select a sector first via Operations → Launch Product.", ephemeral=True)
        sector = row[0]
...
# [Include all other Modals/Views from your original file: DescriptionModal, RenameCompanyModal, SetSalaryModal, InvestModal, UpgradeProductDropdown, MarketingModal, DividendModal, RndInvestModal, BannerModal, PhilosophyDropdown, HireSpecialistDropdown, FireEmployeeDropdown, StartupModal, VPTitleDropdown, ProductPerformanceView, HRView, SettlementView, EspionageModal, etc.]
# For brevity, I'll keep the structure identical to yours but with the new EspionageModal added:
...
class EspionageModal(discord.ui.Modal, title='Corporate Espionage'):
    ...
    type = discord.ui.TextInput(label='Operation Type', placeholder='steal_tech or sabotage_rep')

    async def on_submit(self, i: discord.Interaction):
        try:
            tid = int(self.target_id.value)
            op = self.type.value.strip().lower()
            if op not in ['steal_tech', 'sabotage_rep']: raise ValueError
        except: return await i.response.send_message("❌ Invalid ID or type. Use 'steal_tech' or 'sabotage_rep'.", ephemeral=True)
        
        if tid == i.user.id: return await i.response.send_message("❌ Cannot target yourself.", ephemeral=True)
        
        cost = 50000 if op == 'steal_tech' else 30000
        with get_db_cursor() as c:
            if not atomic_business_update(c, i.user.id, -cost): return await i.response.send_message("❌ Insufficient capital.", ephemeral=True)
            
            c.execute("SELECT tech_level, reputation FROM businesses WHERE user_id = ?", (tid,))
            target = c.fetchone()
            if not target: return await i.response.send_message("❌ Target business not found.", ephemeral=True)
            ...
            if random.random() < success_chance:
                if op == 'steal_tech':
                    ...
                    c.execute("UPDATE businesses SET tech_level = MAX(0, tech_level - 3) WHERE user_id = ?", (tid,))
                    msg = "✅ Espionage successful! +3 Tech points transferred."
                else:
                    c.execute("UPDATE businesses SET reputation = MAX(0, reputation - 15) WHERE user_id = ?", (tid,))
                    msg = "✅ Sabotage successful! Rival reputation -15."
                log_business_event(c, i.user.id, "ESPIONAGE_SUCCESS", f"{op} on {tid}")
            else:
...
class Business(commands.Cog):
    ...
    async def _post_to_channel(self, embed: discord.Embed):
    ...
    async def daily_cycle(self):
        with get_db_cursor() as c, get_eco_cursor() as c_eco:
            ...
            for row in c.fetchall():
                ...
                # Quarter rollover every 28 days
                if biz['days_open'] % 28 == 0:
                    ...
                    asyncio.create_task(self._post_to_channel(embed))
                    # In practice, you'd DM or ping with EarningsCallView. Attached here for reference.
    ...
    async def business_hub(self, i: discord.Interaction):
        ...
        await i.response.send_message("🔄 *𝑐𝑐𝑠𝑠𝑖𝑛 𝐶𝐸𝑂 𝑇𝑟𝑚𝑖𝑛𝑎𝑙...*", ephemeral=False)
        ...
        with get_db_cursor() as c:
            ...
            row = c.fetchone()
            banner_url = row[0] if row else "https://cdn.discordapp.com/attachments/1441473281420169367/1501576429761200290/0ac4c99804a08a107d2cf6f09d79655f.jpg"
