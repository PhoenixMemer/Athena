...
SECTOR_BASE_COSTS = {"Tech": 350, "Food": 250, "Luxury": 3000, "Retail": 300, "Industrial": 1200, "Energy": 1200}

COUNTRY_TAX_RATES = {
    "USA": {"brackets": [(100000, 0.0), (500000, 0.05), (2000000, 0.15), (float('inf'), 0.30)]},
    "UK": {"brackets": [(100000, 0.0), (500000, 0.07), (2000000, 0.18), (float('inf'), 0.35)]},
    "Germany": {"brackets": [(100000, 0.0), (500000, 0.06), (2000000, 0.16), (float('inf'), 0.32)]},
    "France": {"brackets": [(100000, 0.0), (500000, 0.08), (2000000, 0.19), (float('inf'), 0.37)]},
    "Brazil": {"brackets": [(100000, 0.0), (500000, 0.09), (2000000, 0.20), (float('inf'), 0.40)]},
    "China": {"brackets": [(100000, 0.0), (500000, 0.04), (2000000, 0.14), (float('inf'), 0.28)]},
    "Vietnam": {"brackets": [(100000, 0.0), (500000, 0.03), (2000000, 0.12), (float('inf'), 0.15)]}
}

def calculate_progressive_tax(net_profit: int) -> int:
    if net_profit <= 100_000: return 0
    elif net_profit <= 500_000: return int(net_profit * 0.05)
    elif net_profit <= 2_000_000: return int(net_profit * 0.15)
    else: return int(net_profit * 0.30)
...
class DiluteSharesModal(discord.ui.Modal, title='Issue Corporate Shares'):
    ...
    async def on_submit(self, i: discord.Interaction):
        try:
            ...
            await i.response.send_message(f"**Equity Issuance Executed Successfully!**\nYou minted and sold **{shares_to_mint:,} shares** on the open market, raising **A$ {capital_raised:,}** in liquid corporate capital.\n\n**Market Readjustments Applied:**\n• Brand Reputation dropped by **-{rep_drop}%** due to dilution irritation.\n• Stock Price shifted from A$ {current_stock_price:,} down to **A$ {new_stock_price:,}**.", ephemeral=False)
        except ValueError:
...
class ProductModal(discord.ui.Modal):
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        with get_db_cursor() as c:
            ...
            c.execute("INSERT INTO business_products (user_id, name, category, unit_price, cost_to_make, production_target, quality_tier, active) VALUES (?, ?, ?, ?, ?, ?, 'Standard', 1)",
...
class TerminalView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        # Ensure only the person who invoked the command can use the buttons
        if interaction.message.interaction and interaction.user.id != interaction.message.interaction.user.id:
            await interaction.response.send_message("<a:wt_torono:1480580892706603018> Access Denied. This terminal belongs to someone else.", ephemeral=True)
            return False
        return True

    @discord.ui.button(label="Refresh", style=discord.ButtonStyle.primary, emoji="🔄", custom_id="term_refresh", row=4)
    async def refresh_dash(self, i: discord.Interaction, btn):
        await i.response.defer()
        embed, view = await get_dashboard(i.client, i.user.id)
        await i.edit_original_response(embed=embed, view=view)

    @discord.ui.button(label="Balance Sheet", style=discord.ButtonStyle.secondary, row=0, emoji="<:i_cupid:1426518951961038929>", custom_id="term_balance")
    ...
    @discord.ui.button(label="Operations", style=discord.ButtonStyle.secondary, row=0, emoji="<a:i_devils:1426518576784736287>", custom_id="term_ops")
    ...
    @discord.ui.button(label="HR", style=discord.ButtonStyle.secondary, row=0, emoji="<:i_ghouls:1426522093620826112>", custom_id="term_hr")
    ...
    @discord.ui.button(label="R&D Hub", style=discord.ButtonStyle.secondary, row=1, emoji="<:i_incubus:1426520255462903869>", custom_id="term_rnd")
    ...
    @discord.ui.button(label="Rename", style=discord.ButtonStyle.secondary, row=1, emoji="<a:i_pm:1426519079673659493>", custom_id="term_rename")
    async def rename(self, i: discord.Interaction, btn):
        await i.response.send_modal(RenameCompanyModal())

    @discord.ui.button(label="IPO", style=discord.ButtonStyle.secondary, row=1, emoji="<:w_moth2:1380799242515386368>", custom_id="term_ipo")
    ...
    @discord.ui.button(label="Issue Shares (Dilute)", style=discord.ButtonStyle.secondary, row=1, emoji="<a:w_tear:1375482116749529098>", custom_id="term_dilute")
    ...
    @discord.ui.button(label="Marketing", style=discord.ButtonStyle.secondary, row=2, emoji="<:i_satan:1426518223133737000>", custom_id="term_market")
    async def market(self, i: discord.Interaction, btn):
        await i.response.send_modal(MarketingModal())

    @discord.ui.button(label="Pay Dividends", style=discord.ButtonStyle.secondary, row=2, emoji="<:i_succubus:1426518422975549500>", custom_id="term_div")
    async def dividend(self, i: discord.Interaction, btn):
        await i.response.send_modal(DividendModal())

    @discord.ui.button(label="Invest in R&D", style=discord.ButtonStyle.secondary, row=2, emoji="<:i_trialmod:1426518688911069184>", custom_id="term_rnd_inv")
    async def invest_rnd(self, i: discord.Interaction, btn):
        await i.response.send_modal(RndInvestModal())

    @discord.ui.button(label="Espionage", style=discord.ButtonStyle.secondary, row=3, emoji="<:i_lucifer:1426518321544564798>", custom_id="term_esp")
    async def espionage(self, i: discord.Interaction, btn):
        await i.response.send_modal(EspionageModal())

    @discord.ui.button(label="Audit", style=discord.ButtonStyle.secondary, row=3, emoji="<a:c_rolling:1218512150549499904>", custom_id="term_audit")
    async def audit(self, i: discord.Interaction, btn):
        with get_db_cursor() as c:
            c.execute("SELECT capital, owner_salary, days_open, last_report, sector, strike_active, last_audit FROM businesses WHERE user_id = ?", (i.user.id,))
            biz = c.fetchone()
            now = time.time()
            ...
            c.execute("UPDATE businesses SET last_audit = ? WHERE user_id = ?", (now, i.user.id))
            c.execute("SELECT COUNT(id), SUM(salary), AVG(morale) FROM employees WHERE user_id = ?", (i.user.id,))
            emps = c.fetchone()
            emp_count = emps[0] or 0
            avg_morale = emps[2] or 100
            c.execute("SELECT name, unit_price, cost_to_make, production_target, quality_tier FROM business_products WHERE user_id = ? AND active = 1", (i.user.id,))
            prods = c.fetchall()
            advice = []
...
class Business(commands.Cog):
    def __init__(self, bot):
        ...
        self.bot.add_view(TerminalView(bot))
    ...
    async def biz_awardcar(self, i: discord.Interaction, user: discord.Member, car: app_commands.Choice[str]):
        ...
        with get_eco_cursor() as c_eco:
            ...
            c_eco.execute('''INSERT INTO inventory (user_id, item_name, quantity) 
                             ...
                             DO UPDATE SET quantity = quantity + 1''', (user.id, car.value))
    ...
    def _simulate_company_cycle(self, c, c_eco, biz):
        ...
        for p in prods:
            ...
            if margin_ratio > 8.0:
                ...
                sold = int(base_sold * 0.30)
            elif margin_ratio > 1.5: 
                sold = int(base_sold * 0.80)
            else:
                sold = base_sold
    ...
    async def business_hub(self, i: discord.Interaction):
        ...
        await i.edit_original_response(content=None, embed=embed, view=TerminalView(self.bot))
