...
class Investments(commands.Cog):
    ...
    async def dividend_payouts(self):
        ...
        # Apply payouts outside the main read lock to prevent locking economy.db too long
        for uid, amount in user_payouts.items():
            # Use apply_balance_increase from economy.py to trigger Rich Person / Card Upgrades!
    ...
    async def buy(self, interaction: discord.Interaction, symbol: str, shares: int):
        ...
        if capital_injection > 0:
            try:
                with get_db_cursor() as cur_eco:
                    cur_eco.execute("SELECT name FROM stocks WHERE symbol = ?", (sym,))
    ...
    async def sell(self, interaction: discord.Interaction, symbol: str, shares: int):
        ...
        # ✅ FIX: Use apply_balance_increase to trigger Rich Person and card upgrades!
        await apply_balance_increase(interaction.user.id, total_value, interaction.channel, tx_type="sell_stock")
