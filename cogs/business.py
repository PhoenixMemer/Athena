...
class ProductPerformanceView(discord.ui.View):
    ...
    def get_embed(self):
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT name, category, unit_price, cost_to_make, lifetime_revenue FROM business_products WHERE user_id = ?", (self.user_id,))
        prods = c.fetchall()
        conn.close()

        embed = discord.Embed(title="꒰ა ﹒chérie  ⸝⸝", color=0xffffff)
        if not prods:
            embed.description = "<a:wt_toroconfused:1480580932367945918> 𝑌𝑜𝑢 ℎ𝑎𝑣𝑒 𝑛𝑜 𝑎𝑐𝑡𝑖𝑣𝑒 𝑝𝑟𝑜𝑑𝑢𝑐𝑡 𝑙𝑖𝑛𝑒𝑠. 𝐿𝑎𝑢𝑛𝑐ℎ 𝑜𝑛𝑒 𝑖𝑛 𝑂𝑝𝑒𝑟𝑎𝑡𝑖𝑜𝑛𝑠."
            return embed

        desc = "**<a:wt_torolove:1480580899430203484> Product Portfolio & Performance**\n\n"
        for name, cat, price, cost, rev in prods:
            margin = price - cost
            desc += (
                f"**<:s_white2:1382052523166142486> {name}** ({cat})\n"
                f"└─ Price: A$ {price:,} | Margin: A$ {margin:,}\n"
                f"└─ Lifetime Revenue: **A$ {rev:,}**\n\n"
            )
        embed.description = desc
        return embed

# ==========================================
# ⚖️ STRIKE SETTLEMENT UI
# ==========================================
class SettlementView(discord.ui.View):
    ...
    async def pay_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if cap < 100000:
            conn.close()
            return await interaction.response.send_message("<a:wt_torono:1480580892706603018> Your business is too broke to settle.", ephemeral=True)
        ...
        await interaction.response.send_message("<a:wt_toroexclaim:1480581004317036624> Settlement paid. The strike has ended.", ephemeral=False)

    @discord.ui.button(label="Grant 15% Permanent Raise", style=discord.ButtonStyle.success)
    async def raise_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        conn.commit(); conn.close()
        await interaction.response.send_message("<a:wt_torolove:1480580899430203484> Salaries increased. Employees have returned to work.", ephemeral=False)

# ==========================================
# 🛠️ UPDATED TERMINAL VIEW (Add these to your existing TerminalView)
# ==========================================
# [Inside your TerminalView class, add these two methods:]

    @discord.ui.button(label="R&D Hub", style=discord.ButtonStyle.secondary, row=2)
    async def rnd_hub_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        view = ProductPerformanceView(interaction.user.id)
        await interaction.response.send_message(embed=view.get_embed(), view=view, ephemeral=True)

    @discord.ui.button(label="Resolve Strike", style=discord.ButtonStyle.danger, row=2)
    async def strike_resolve_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        conn = sqlite3.connect(DB_PATH); c = conn.cursor()
        c.execute("SELECT AVG(morale) FROM employees WHERE user_id = ?", (interaction.user.id,))
        avg_morale = c.fetchone()[0] or 100
        conn.close()
        
        if avg_morale >= 20:
            return await interaction.response.send_message("<a:wt_toroexclaim:1480581004317036624> 𝑌𝑜𝑢𝑟 𝑤𝑜𝑟𝑘𝑓𝑜𝑟𝑐𝑒 𝑖𝑠 𝑛𝑜𝑡 𝑜𝑛 𝑠𝑡𝑟𝑖𝑘𝑒.", ephemeral=True)
            
        embed = discord.Embed(title="꒰ა ﹒chérie  ⸝⸝", color=0xffffff)
        embed.description = "⚠️ **UNION DEMANDS:** Your employees are striking. Production is halted. Choose a resolution path below."
...
class Business(commands.Cog):
    ...
    def setup_db(self):
        ...
        # ==========================================
        # 🛡️ FAIL-SAFE SCHEMA UPDATER
        # Automatically injects new Mark 15.4 columns into old databases
        # ==========================================
        new_columns = [
            ("businesses", "vp_id", "INTEGER"),
            ("businesses", "vp_title", "TEXT"),
            ("businesses", "demand_boost", "REAL DEFAULT 1.0"),
            ("businesses", "hq_level", "INTEGER DEFAULT 0"),
            ("businesses", "philosophy", "TEXT DEFAULT 'Mass Market'"),
            ("businesses", "reputation", "INTEGER DEFAULT 100"),
            ("businesses", "is_public", "INTEGER DEFAULT 0"),
            ("employees", "specialization", "TEXT DEFAULT 'None'")
        ]
